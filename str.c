...
string str_slice(string s, size_t start, size_t length) 
{
...
string str_vprintf(Arena *arena, char* fmt, va_list args)
{
    va_list args2;
    va_copy(args2, args);
    int needed_bytes = bnprintf(NULL, 0, fmt, args) + 1;
    string result = {0};
...
string str_print(Arena *arena, char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
