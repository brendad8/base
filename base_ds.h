...
// T*         HMAP_GET_PTR_TS    (T* map, TK key, ptrdiff_t tmp)    Same at HMAP_GET_PTR but thread safe
// T*         HMAP_GET_PTR_NULL  (T* map, TK key)                   Same as HMAP_GET_PTR but retunrs null if key is missing
//
// ptrdiff_t  HMAP_GET_IDX       (T* map, TK key)                   Returns the index of key into map array or -1 if key is missing
// ptrdiff_t  HMAP_GET_IDX_TS    (T* map, ptrdiff_t tmp)            Same as HMAP_GET_IDX but thread safe
//
...
// size_t     SMAP_LEN           (T* map)                         Returns number of items in map. Can be used to iterate over items in map array.
//
// void       SMAP_DEFAULT       (T* map, TV value)               Sets the default value returned by SMAP_GET if key is not present
// void       SMAP_DEFAULTS      (T* map, T struct)               Sets the default struct returned by SMAP_GETS if key is not present
//
// void       SMAP_PUT           (T* map, char* key, TV value)    Update existing key, value or add new item if key is new         
// void       SMAP_PUTI          (T* map, char* key, TV value)    Same as SMAP_PUT but returns index of item into item array
// void       SMAP_PUTS          (T* map, T struct)               Same as SMAP_PUT but takes in struct with key and value fields
//
// TV         SMAP_GET           (T* map, char* key)              Returns the value of the corresponding key, or default if key is missing
// T          SMAP_GETS          (T* map, char* key)              Returns the struct of the corresponding key, or default if key is missing
//
// T*         SMAP_GET_PTR       (T* map, char* key)              Returns pointer to struct corresponding to key or pointer to default if key is missing
// T*         SMAP_GET_PTR_NULL  (T* map, char* key)              Same as SMAP_GET_PTR but retunrs null if key is missing
//
// ptrdiff_t  SMAP_GET_IDX       (T* map, char* key)              Returns the index of key into map array or -1 if key is missing
//
// int        SMAP_DEL           (T* map, char* key)              Deletes item corresponding to key from hashmap and returns 1. Returns 0 otherwise.
//
// void       SMAP_FREE          (T* map)                         Frees hash map memory and sets pointer to NULL


#define SMAP_NEW_ARENA     stbds_sh_new_arena
#define SMAP_NEW_STRDUP    stbds_sh_new_strdup

#define SMAP_LEN           stbds_shlenu
...
#define SMAP_PUT_IDX       stbds_shputi
#define SMAP_GET           stbds_shget
#define SMAP_GETS          stbds_shgets
#define SMAP_GET_PTR       stbds_shgetp
#define SMAP_GET_PTR_NULL  stbds_shgetp_null
#define SMAP_GET_IDX       stbds_shgeti
